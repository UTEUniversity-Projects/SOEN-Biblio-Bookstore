      ...
      <div class="cr-breadcrumb">
        <div class="d-flex align-items-center">
          <div class="page-title">
            <h5><b>Danh sách danh mục</b></h5>
          </div>
          <a href="${pageContext.request.contextPath}/owner/category/create" class="cr-btn default-btn color-primary">
            Thêm danh mục
          </a>
        </div>
        <ul>
          <li><a href="${pageContext.request.contextPath}/owner/ecommerce"><b>Biblio</b></a></li>
          <li><b>Danh sách danh mục</b></li>
        </ul>
      </div>
    </div>
    <div class="row">
            ...
            <div class="table-responsive">
              <table id="customer-data-table" class="item-data-table table table-hover rounded-table responsive-table">
                ...
                <c:forEach var="category" items="${categories}">
                  <tr class="item-row" id="row-${category.id}" data-id="${category.id}">
                    <td>
                      <img
                    ...
                    <td>
                      <div>
                        <span class="row-highlight">${category.name}</span><br>
                        <span class="row-sub-content">${category.shortScript}</span>
                      </div>
                    </td>
                    <td><span class="badge bg-primary">${category.works}</span></td>
                    <td>
                      <div class="d-flex align-items-center">
                        <h6 class="avg-rate">${category.avgRate}</h6>
                        ...
                        <c:choose>
                          <c:when test="${category.perValueBooksSold == 0}">
                            <div class="up mt-0 d-flex">
                              ...
                              <p class="non-growth">${category.perValueBooksSold}%</p>
                            </div>
                          </c:when>
                          <c:when test="${category.perValueBooksSold > 0}">
                            <div class="up mt-0 d-flex">
                              <i class="ri-arrow-up-line me-1 icon-growth"></i>
                              <p class="up-growth">${category.perValueBooksSold}%</p>
                            </div>
                          </c:when>
                          <c:otherwise>
                            <div class="down mt-0 d-flex">
                              <i class="ri-arrow-down-line me-1 icon-growth"></i>
                              <p class="down-growth">${-category.perValueBooksSold}%</p>
                            </div>
                          </c:otherwise>
                        </c:choose>
                      </div>
                    ...
                    <td>
                      <span class="row-fullScript">${category.fullScript}</span>
                    </td>
                    <td>
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="dropdown-menu">
                          <a class="dropdown-item" href="#" onclick="rowAction(${category.id}, 'view'); return false;" data-value="view">Chi tiết</a>
                          <a class="dropdown-item" href="#" onclick="rowAction(${category.id}, 'update'); return false;" data-value="update">Cập nhật</a>
                          <a class="dropdown-item" href="#" onclick="rowAction(${category.id}, 'delete'); return false;" data-value="delete">Xóa</a>
                        </div>
                      </div>
                    </td>
                  </tr>
