...
$(document).ready(() => {
    ...
    const lazyLoadElements = document.querySelectorAll('.section-related-products');
    ...
    const generateBook = (book) => {
        return `
        <div class="col-xxl-2 col-xl-3 col-md-4 col-sm-6 col-12 mb-5">
    ...
    const getBooks = async () => {
        const bookTemplateId = $('.section-product').data('book-id');
        const bookList = document.querySelector('.cr-popular-product');

        await $.ajax({
            url: `${contextPath}/api/customer/load-related-books`,
            type: 'POST',
