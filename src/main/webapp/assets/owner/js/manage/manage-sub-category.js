...
function deleteActionProcess(itemId) {
  fetch(`${contextPath}/api/owner/sub-category/delete`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      .then((data) => {
        if (data.can === "true") {
          ...
          showToast("Không thể xóa vì có " + data.amount + " sản phẩm cùng phân loại", "Cảnh báo", "warning");
        }
      })
      .catch((error) => {
        console.error("Error:", error);
        showToast("Có lỗi xảy ra khi xóa phân loại!", "Lỗi", "error");
      });
}

// Executes delete action using HTTP DELETE
function executeDeleteAction(itemId) {
  fetch(`${contextPath}/owner/sub-category/delete`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      ...
      .then((data) => {
        if (data.status === "success") {
          ...
          showToast("Xóa phân loại thành công!", "Thành công", "success");
        } else {
          showToast("Xóa phân loại thất bại!", "Lỗi", "error");
        }
      })
      .catch((error) => {
        console.error("Error:", error);
...
function executeViewOrUpdateAction(itemId, action) {
  fetch(`${contextPath}/session/owner/sub-category/set-info`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      ...
      subCategoryAction: action,
    }),
  })
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          window.location.href = `${contextPath}/owner/sub-category/${action}`;
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const handleCreateSubCategory = async (event) => {
    ...
    try {
      const formData = new FormData(document.getElementById("subCategoryCreateForm"));
      ...
      const response = await fetch(`${contextPath}/owner/sub-category/create`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      if (result.status === "success") {
        showToast("Thêm phân loại thành công!", "Thành công", "success", 2000);
        setTimeout(() => {
          rowAction(result.id, 'view');
        }, 1000);
      } else {
        showToast("Thêm phân loại thất bại!", "Lỗi", "error");
        console.error("Create failed:", result.message || response.statusText);
      }
    } catch (error) {
      showToast("Đã xảy ra lỗi khi thêm phân loại!", "Lỗi", "error");
      console.error("Error during the subCategory process:", error);
    }
  };
  ...
  const handleUpdateSubCategory = async (event) => {
    ...
    try {
      const formData = new FormData(document.getElementById("subCategoryUpdateForm"));
      ...
      const response = await fetch(`${contextPath}/owner/sub-category/update`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      if (result.status === "success") {
        showToast("Cập nhật phân loại thành công!", "Thành công", "success");
        setTimeout(() => {
          rowAction(subCategoryData.id, "view");
        }, 1000);
      } else {
        showToast("Cập nhật phân loại thất bại!", "Lỗi", "error");
        console.error("Update failed:", result.message || response.statusText);
      }
    } catch (error) {
      console.error("Error during the subCategory update process:", error);
      showToast("Đã xảy ra lỗi khi cập nhật phân loại!", "Lỗi", "error");
    }
  };

  const createButton = document.getElementById("createButton");
  if (createButton != null) createButton.addEventListener("click", handleCreateSubCategory);

  const updateButton = document.getElementById("updateButton");
  if (updateButton != null) updateButton.addEventListener("click", handleUpdateSubCategory);
});
