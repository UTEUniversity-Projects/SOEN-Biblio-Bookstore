...
function deleteActionProcess(itemId) {
  fetch(`${contextPath}/api/owner/translator/delete`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      .then((data) => {
        if (data.can === "true") {
          ...
          showToast("Không thể xóa vì có " + data.amount + " sản phẩm cùng dịch giả", "Cảnh báo", "warning");
        }
      })
      .catch((error) => {
...
function executeDeleteAction(itemId) {
  fetch(`${contextPath}/owner/translator/delete`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      ...
      .then((data) => {
        if (data.status === "success") {
          ...
          showToast("Xóa dịch giả thành công!", "Thành công", "success");
        } else {
          showToast("Xóa dịch giả thất bại!", "Lỗi", "error");
        }
      })
      .catch((error) => {
        console.error("Error:", error);
        showToast("Có lỗi xảy ra khi xóa dịch giả!", "Lỗi", "error");
      });
}

// Handles view or update action and redirects based on server response
function executeViewOrUpdateAction(itemId, action) {
  fetch(`${contextPath}/session/owner/translator/set-info`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      translatorId: itemId,
      translatorAction: action,
    }),
  })
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          window.location.href = `${contextPath}/owner/translator/${action}`;
        } else {
  ...
  if (tableBody) {
    tableBody.addEventListener("click", function (event) {
      ...
      if (row && !event.target.closest("button") && !event.target.closest(".dropdown-menu")) {
        const translatorId = row.getAttribute("data-id");
        rowAction(translatorId, "view");
      }
    });
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const handleCreateTranslator = async (event) => {
    ...
    try {
      const form = document.getElementById("translatorCreateForm");
      const formData = new FormData(form);

      const introduction = editor.root.innerHTML.trim();
      if (!formData.get("name") || !introduction || introduction === "<p><br></p>") {
        showToast("Tên dịch giả và giới thiệu không được để trống!", "Lỗi", "error");
        return;
      }

      let avatar = await uploadImage("translator", "#avatar");
      avatar = avatar || "/images/anonymous/translator.jpg";

      formData.append("avatar", avatar);
      formData.append("introduction", introduction);
      const translatorData = Object.fromEntries(formData.entries());

      const response = await fetch(`${contextPath}/owner/translator/create`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify(translatorData)
      });
      ...
      showToast("Đã xảy ra lỗi khi thêm dịch giả!", "Lỗi", "error");
      console.error("Error during the translator process:", error);
    }
  };
  ...
  const handleUpdateTranslator = async (event) => {
    ...
    try {
      const formData = new FormData(document.getElementById("translatorUpdateForm"));
      ...
      if (avatarInput.files && avatarInput.files.length > 0) {
        avatar = await uploadImage("translator", "#avatar");
        if (!avatar) {
      ...
      const response = await fetch(`${contextPath}/owner/translator/update`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      if (result.status === "success") {
        showToast("Cập nhật dịch giả thành công!", "Thành công", "success");
        setTimeout(() => {
          rowAction(translatorData.id, "view");
        }, 1000);
      } else {
        showToast("Cập nhật dịch giả thất bại!", "Lỗi", "error");
        console.error("Update failed:", result.message || response.statusText);
      }
    } catch (error) {
      console.error("Error during the translator update process:", error);
      showToast("Đã xảy ra lỗi khi cập nhật dịch giả!", "Lỗi", "error");
    }
  };
